...
HONEST LIMITATION: no real polling-aggregator API (538, RealClearPolitics, etc.)
is integrated anywhere in this codebase. What follows is a PRICE-TREND PROXY:
election/political markets are filtered by keyword, and this bot's own rolling
price history (strategies.base.record_price_sample / monotonic_trend — same
mechanism as S12 Momentum, see that file for the warm-up/in-process caveats) is
used as a stand-in for polling drift, on the theory that a sustained price move in
an election market is itself downstream of real polling/news updates working
their way into the crowd's pricing. This is NOT a real polling feed — it is
distinguished from plain S12 Momentum only by (a) being scoped to
election/political markets and (b) using a longer lookback, since poll-driven
repricing plays out slower than generic momentum.
"""
...
ELECTION_KEYWORDS = (
    "election", "president", "senate", "governor", "primary", "electoral",
    "ballot", "runoff", "parliament", "prime minister", "referendum", "caucus",
    "re-election", "reelection",
)

def _is_election_market(question: str, tags: list) -> bool:
    q = (question or "").lower()
    if any(kw in q for kw in ELECTION_KEYWORDS):
        return True
    tag_labels = [(t.get("label") or "").lower() for t in (tags or [])]
    return any(("politic" in t) or ("election" in t) for t in tag_labels)
...
class PollDriftStrategy(BaseStrategy):
    ...
    async def scan(self, markets: List[dict], balance: float, http_client: httpx.AsyncClient) -> List[dict]:
        ...
        trend_threshold_pct = await cfg.get_typed(f"{self.key}.trend_threshold_pct", float, 4.0)
        lookback_s = await cfg.get_typed(f"{self.key}.lookback_s", float, 21600.0)  # 6h — slower than S12
        ...
        assumed_hold_days = await cfg.get_typed(f"{self.key}.assumed_hold_days", float, 5.0)
        ...
        for market in markets:
            try:
                if not _is_election_market(market.get("question"), market.get("tags")):
                    continue
                ...
                if trend is None:
                    continue  # warm-up, or no sustained drift yet
                ...
                if trend == "up":
                    target_idx = yes_idx
                    thesis = f"YES drifted up {change_pct:.1f}% over the last {lookback_s/3600:.1f}h"
                else:
                    target_idx = no_idx
                    thesis = f"YES drifted down {abs(change_pct):.1f}% over the last {lookback_s/3600:.1f}h (NO trending up)"
                ...
                opps.append({
                    ...
                    "status": "open",
                    "notes": (
                        f"Poll-drift PROXY (price-trend derived, NOT a real polling aggregator feed): "
                        f"{thesis} — buying '{outcomes[target_idx]}' at ${real_price:.4f}."
                    ),
                    "instructions": [
                        f"Open: {market_url}",
                        f"Buy '{outcomes[target_idx]}' for ${round(suggested_usdc, 2)} USDC at ~${real_price:.4f}",
                        "Cross-check against real polling data before trusting this price-trend proxy."
                    ]
                })
            except Exception:
                continue
